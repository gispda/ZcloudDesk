...
* AES - CBC
*******************/
void aes_encrypt_cbc(const BYTE in[], size_t in_len,const BYTE * lastBlock, BYTE out[], const ZWORD key[], int keysize, const BYTE iv[])
{
...
void aes_decrypt_cbc(const BYTE in[], size_t in_len, BYTE out[], const ZWORD key[], int keysize, const BYTE iv[])
{
	BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
	int blocks, idx;
...
int aes_encrypt_cbc_mac(const BYTE in[], size_t in_len, BYTE out[], const ZWORD key[], int keysize, const BYTE iv[])
{
    BYTE buf_in[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE], iv_buf[AES_BLOCK_SIZE];
    int blocks, idx;
...
void aes_encrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const ZWORD key[], int keysize, const BYTE iv[])
{
...
void aes_decrypt_ctr(const BYTE in[], size_t in_len, BYTE out[], const ZWORD key[], int keysize, const BYTE iv[])
{
...
int aes_encrypt_ccm(const BYTE payload[], ZWORD payload_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE out[], ZWORD *out_len,
                    ZWORD mac_len, const BYTE key_str[], int keysize)
{
...
int aes_decrypt_ccm(const BYTE ciphertext[], ZWORD ciphertext_len, const BYTE assoc[], unsigned short assoc_len,
                    const BYTE nonce[], unsigned short nonce_len, BYTE plaintext[], ZWORD *plaintext_len,
                    ZWORD mac_len, int *mac_auth, const BYTE key_str[], int keysize)
{
...
void aes_key_setup(const BYTE key[], ZWORD w[], int keysize)
{
	int Nb=4,Nr,Nk,idx;
	ZWORD temp,Rcon[]={0x01000000,0x02000000,0x04000000,0x08000000,0x10000000,0x20000000,
...
void AddRoundKey(BYTE state[][4], const ZWORD w[])
{
	BYTE subkey[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const ZWORD key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt(const BYTE in[], BYTE out[], const ZWORD key[], int keysize)
{
	BYTE state[4][4];
