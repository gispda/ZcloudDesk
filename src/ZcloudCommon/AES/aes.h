...
typedef unsigned int ZWORD;             // 32-bit word, change to "long" for 16-bit machines
...
void aes_key_setup(const BYTE key[],          // The key, must be 128, 192, or 256 bits
                   ZWORD w[],                  // Output key schedule to be used later
                   int keysize);              // Bit length of the key, 128, 192, or 256

void aes_encrypt(const BYTE in[],             // 16 bytes of plaintext
                 BYTE out[],                  // 16 bytes of ciphertext
                 const ZWORD key[],            // From the key setup
                 int keysize);                // Bit length of the key, 128, 192, or 256
...
int aes_encrypt_ccm(const BYTE plaintext[],              // IN  - Plaintext.
                    ZWORD plaintext_len,                  // IN  - Plaintext length.
                    ...
                    ZWORD *ciphertext_len,                // OUT - The length of the ciphertext, always plaintext_len + mac_len.
                    ZWORD mac_len,                        // IN  - The desired length of the MAC, must be 4, 6, 8, 10, 12, 14, or 16.
...
int aes_decrypt_ccm(const BYTE ciphertext[],             // IN  - Ciphertext, the concatination of encrypted plaintext and MAC.
                    ZWORD ciphertext_len,                 // IN  - Ciphertext length in bytes.
                    ...
                    ZWORD *plaintext_len,                 // OUT - Length in bytes of the output plaintext, always ciphertext_len - mac_len .
                    ZWORD mac_len,                        // IN  - The length of the MAC that was calculated.
