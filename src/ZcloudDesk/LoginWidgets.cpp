...
bool LoginWidget::eventFilter(QObject *target, QEvent *e)
{
	if (e->type() == QEvent::MouseButtonRelease)
	{
		if (target == registerLael)
		{
			...
			//注册
			changeLoginMethod(3);
			statusLoginLabel->text() != QString::fromLocal8Bit("登录/注册") ?
		...
		else if (target == statusLoginLabel_2)
		{
			...
			changeLoginMethod(statusLoginLabel->text() != QString::fromLocal8Bit("登录/注册") ? 1 : 2);
		}
		...
		else if (target == verificationCodeLabel)
		{
			...
			else
			{
				...
				{
					QString strCodeType;
					if (statusLoginLabel->text() == QString::fromLocal8Bit("登录/注册"))//手机登录
					{
...
void LoginWidget::changeLoginMethod(int statusFlag, bool isClear)
{
	...
	{
		//手机验证码登录
		statusLoginLabel->setText(QString::fromLocal8Bit("登录/注册"));
	...
	case 2:
	{
		...
		statusLoginLabel_2->setText(QString::fromLocal8Bit("<html><head/><body><p><span style=\" font:14px 'Microsoft YaHei';  font-weight:600;color:#666666;\">登录/注册</span>"
			"<span style=\" font:14px 'Microsoft YaHei'; font-weight:600; color:#1f8bed;\">&gt;</span></p></body></html>"));
...
void LoginWidget::onLoginButtonClicked()
{
	...
	{
		if (statusLoginLabel->text() != QString::fromLocal8Bit("登录/注册"))
		{
			//账号密码登录
